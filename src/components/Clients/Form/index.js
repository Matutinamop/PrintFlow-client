...
function ClientsForm({
  ...
  useEffect(() => {
    if (isEdit && client && Object.keys(client).length > 0) {
  ...
  const addContact = () => {
    ...
    const newContacts = [...oldContacts, { name: '', phone: '', email: '' }];
    setFields((prev) => ({
      ...prev,
      contact: newContacts,
    }));
  ...
  const deleteContact = (i) => {
    const newContacts = fields.contact.filter((range, index) => i !== index);
    setFields((prev) => ({
      ...prev,
      contact: newContacts,
    }));
  ...
  const handleSubmit = async () => {
    ...
    if (!isEdit) {
      try {
        ...
        if (setQuickClient) {
          console.log('estoy desde el clientForm', res);
