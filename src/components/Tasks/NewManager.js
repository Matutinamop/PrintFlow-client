...
import { toFormatDate, warningLevel } from '../../utilities/functions/dates';
...
function NewManager() {
  ...
  return (
    <div className={styles.taskManager} ref={taskManagerRef}>
      <Modal isOpen={moveModal.isOpen} onClose={cancelMove}>
        <div className={styles.modal}>
          <p className={styles.modalTitle}>
            Mover <strong>{moveModal.draggedEl?.product}</strong> a {''}
          ...
          <div className={styles.modalButtons}>
            <Button variant='contained' color='success' onClick={handleMove}>
              Aceptar
            </Button>
            <Button variant='contained' color='error' onClick={cancelMove}>
              Cancelar
            </Button>
          </div>
        </div>
      ...
      <Modal isOpen={moveModal.bill} onClose={cancelMove}>
        <div className={styles.modal}>
          <p className={styles.modalTitle}>
            ...
            <strong>{moveModal.destination?.name}</strong> es porque ésta ya
            está completa
          </p>
          <label className={styles.modalDescription}>
            Quieres dejar un comentario?
      ...
      >
        <div className={styles.modal}>
          <p className={styles.modalTitle}>
            Necesita ingresar numero de factura para continuar
          </p>
          <input
          ...
          <div className={styles.modalButtons}>
            <Button
              ...
              onClick={() => {
                ...
                handleMove();
                billOrder(moveModal.draggedEl._id, moveModal.billNumber);
              }}
  ...
  return (
    <div className={styles.container} ref={ref}>
      ...
      {tasks?.map((task) => {
        ...
        const levelClass =
          styles[`warningLevel${warningLevel(task)}` || styles.warningLevel0];
        ...
        return (
          ...
          >
            ...
            <div className={styles.content}>{task?.client.companyName}</div>
            ...
            {task.status === 'Detenida' ? (
              <div className={styles.stopedTask}>DETENIDA</div>
            ) : (
              ''
            )}
          </div>
