...
function OrderInfoModule({
  ...
  const formatOptionLabel = ({ label, legalName, address, phone }) => (
  ...
  const selectClient = (option, actionMeta) => {
    ...
    if (actionMeta.action === 'create-option') {
      setClientModalFields({ companyName: option.label, contact: [] });
  ...
  return (
    <div className={styles.blockContainer}>
      <Modal isOpen={clientModalOpen} onClose={() => setClientModalOpen(false)}>
        <ClientsForm
        ...
        <CreatableSelect
          ...
          value={
            clientOptions && Array.isArray(clientOptions)
              ? clientOptions.find(
                  (client) => client.value === fields?.client._id
                ) ?? {
